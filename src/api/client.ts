...
export const api = {
    ...
    lists: {
        ...
        update: async (listId: string, updates: { name?: string; status?: 'draft' | 'shared' | 'completed' }) => {
    ...
    items: {
        ...
        // Add item to list
        add: async (listId: string, itemData: { name: string; quantity?: number; unit?: string; notes?: string }) => {
        ...
        delete: async (listId: string, itemId: string) => {
            const { data } = await apiClient.delete<{ message: string }>(`/lists/${listId}/items/${itemId}`);
            return data;
        },
    },
    ...
    share: {
        ...
        generateLink: async (listId: string, shopkeeperName?: string) => {
            const { data } = await apiClient.post<{ shareToken: string; shareUrl: string }>(`/lists/${listId}/share`, { shopkeeperName });
            return data;
        },
        ...
        acceptShare: async (shareToken: string, shopkeeperName?: string) => {
            const { data } = await axios.post<ShareData>(`${API_BASE_URL}/share/${shareToken}/accept`, { shopkeeperName });
            return data;
        },

        // Shopkeeper: Update list status (public)
        updateListStatus: async (shareToken: string, status: 'draft' | 'shared' | 'completed') => {
            const { data } = await axios.post<GroceryList>(`${API_BASE_URL}/share/${shareToken}/status`, { status });
            return data;
        },

        // Shopkeeper: Update item status (public)
        updateItemStatus: async (shareToken: string, itemId: string, updates: { status: 'to_buy' | 'in_progress' | 'done' | 'unavailable' | 'substituted'; notes?: string }) => {
            const { data } = await axios.post<ListItem>(`${API_BASE_URL}/share/${shareToken}/items/${itemId}/status`, updates);
            return data;
        },
    }
};
