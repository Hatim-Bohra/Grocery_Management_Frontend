...
const getStatusColor = (status: string) => {
  switch (status) {
    case 'to_buy':
      return 'border-gray-300 bg-white text-transparent';
    case 'in_progress':
      return 'border-blue-500 bg-blue-50 text-blue-500';
    case 'done':
      return 'border-green-500 bg-green-500 text-white';
    case 'unavailable':
      return 'border-red-300 bg-red-50 text-red-300';
    case 'substituted':
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-[400px] text-center px-6">
            ...
            <p className="text-gray-500 mb-8 max-w-xs">
              Tap the + button below to add your groceries.
            </p>
          </div>
        ) : (
            ...
            {items.map((item) => {
              ...
              return (
                <div
                  ...
                  className={`relative group bg-white rounded-2xl p-4 shadow-sm border transition-all duration-200 ${
                    isDone
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3
                        className={`text-lg font-semibold leading-tight transition-all ${isDone ? 'text-gray-400 line-through decoration-2' : 'text-gray-800'}`}
                      >
                        {item.name}
                      </h3>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {(item.quantity || item.unit) && (
                          <span
                            className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-md ${isDone ? 'bg-gray-100 text-gray-400' : 'bg-blue-50 text-blue-700'}`}
                          >
                            <Scale size={12} />
                            {item.quantity} {item.unit}
                          </span>
                        ...
                        {item.notes && (
                          <span
                            className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-md ${isDone ? 'bg-gray-100 text-gray-400' : 'bg-amber-50 text-amber-700'}`}
                          >
                            <AlignLeft size={12} />
                            {item.notes}
                          </span>
      ...
      >
        ...
        <div className="flex items-center gap-2 bg-gray-100 p-2 rounded-xl mb-6 border border-gray-200">
          <input
            ...
            readOnly
            value={shareLink}
            className="bg-transparent flex-1 text-sm px-2 outline-none text-gray-600 truncate"
          />
          <button
            onClick={() => {
            ...
            className="bg-white shadow-sm border border-gray-200 px-4 py-2 rounded-lg text-sm font-bold text-blue-600 hover:bg-gray-50"
          >
            Copy
          </button>
        </div>
